...
IHD float3 trace(const Ray ray, const Scene scene) {
  int which;
...
struct GPUScene : Scene {};

GPUScene to_gpu(Scene scene) {
  GPUScene gpu_scene;

  gpu_scene.n_triangles = scene.n_triangles;
  gpu_scene.n_lights    = scene.n_lights;
  cudaMalloc((void**) &gpu_scene.lights, scene.n_lights * sizeof(int));
  cudaMalloc((void**) &gpu_scene.triangles, scene.n_triangles * sizeof(Triangle));
  cudaMalloc((void**) &gpu_scene.normals, scene.n_triangles * sizeof(TriangleNormal));
  cudaMalloc((void**) &gpu_scene.emissivities, scene.n_triangles * sizeof(TriangleEmissivity));
  cudaCheckError();

  cudaMemcpy(gpu_scene.lights, scene.lights, scene.n_lights * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_scene.triangles, scene.triangles, scene.n_triangles * sizeof(Triangle), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_scene.normals, scene.normals, scene.n_triangles * sizeof(TriangleNormal), cudaMemcpyHostToDevice);
  cudaMemcpy(gpu_scene.emissivities, scene.emissivities, scene.n_triangles * sizeof(TriangleEmissivity),
...
__global__ void render_kernel(const Camera camera, const GPUScene scene, uint3 param, uchar4* pixels) {
...
Image render(const Camera camera, const Scene scene, int x, int y, int spp) {
  ...
  render_kernel<<<blocks, threads>>>(camera, to_gpu(scene), make_uint3(x, y, spp), gpu_data);
